if instance_exists(my_player) {
	...
	if gamepad_button_check_pressed(my_device,gp_start) or gamepad_button_check_pressed(my_device,gp_face1) {
	...
	/*if keyboard_check_pressed(vk_enter) and (my_player = obj_player1 or my_player = obj_player2 or my_player = obj_player3) {
		var inst = instance_create_layer(x,y,"player",my_player);
		ds_list_add(global.players_list,inst.object_index);
		sprite_index = noone;
		if instance_exists(obj_start)  obj_start.visible = true;
